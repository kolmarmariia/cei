...
foreach (<FT>){ 
		...
		my @r=split(/;/,$_);
		my $key=$r[3];
			$number_marsh{$key}=$r[0]." ".$r[1];
			$number_marsh{$key}=~s/Трамвай/Tm/g;
			$number_marsh{$key}=~s/Троллейбус/Tb/g;
			#$number_marsh{$key}=~s/minibus/a/g;
			$number_marsh{$key}=~s/Автобус/A/g;
